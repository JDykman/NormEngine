...
set PDBS_DIR=%OUT_DIR%\core_pdbs

set EXE=core_hot_reload.exe

:: Check if core is running
FOR /F %%x IN ('tasklist /NH /FI "IMAGENAME eq %EXE%"') DO IF %%x == %EXE% set CORE_RUNNING=true

if not exist %OUT_DIR% mkdir %OUT_DIR%

:: If core isn't running then:
:: - delete all core_XXX.dll files
:: - delete all PDBs in pdbs subdir
:: - optionally create the pdbs subdir
:: - write 0 into pdbs\pdb_number so core.dll PDBs start counting from zero
::
:: This makes sure we start over "fresh" at PDB number 0 when starting up the
:: core and it also makes sure we don't have so many PDBs laying around.
...
:: Load PDB number from file, increment and store back. For as long as the core
...
:: Build core dll, use pdbs\core_%PDB_NUMBER%.pdb as PDB name so each dll gets
...
:: the debugger is attached. So each time we compile `core.dll` we give the
:: PDB a unique PDB.
:: 
:: Note that we could not just rename the PDB after creation; the DLL contains a
:: reference to where the PDB is.
::
:: Also note that we always write core.dll to the same file. core_hot_reload.exe
:: monitors this file and does the hot reload when it changes.
echo Building core.dll
odin build core -strict-style -vet -debug -build-mode:dll -out:%OUT_DIR%/core.dll -pdb-name:%PDBS_DIR%\core_%PDB_NUMBER%.pdb > nul
IF %ERRORLEVEL% NEQ 0 exit /b 1

:: If core.exe already running: Then only compile core.dll and exit cleanly
if %CORE_RUNNING% == true (
	echo Hot reloading... && exit /b 0
)

:: Build core.exe, which starts the program and loads core.dll och does the logic for hot reloading.
echo Building %EXE%
odin build core\main_core_hot -strict-style -vet -debug -out:%EXE% -pdb-name:%OUT_DIR%\main_hot_reload.pdb
IF %ERRORLEVEL% NEQ 0 exit /b 1
...
@REM 	) else (
@REM 		echo "Please copy raylib.dll from <your_odin_compiler>/vendor/raylib/windows/raylib.dll to the same directory as core.exe"
@REM 		exit /b 1
@REM 	)
@REM )
